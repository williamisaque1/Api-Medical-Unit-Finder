...
app.post("/cordenadas", async (req, res) => {
  ...
  async function loadData() {
    ...
    for (let element = 0; element < results.data.results.length; element++) {
      ...
      data.push(
        axios.get(
          `https://maps.googleapis.com/maps/api/directions/json?origin=${origin.longitude}&destination=${resultado[element].lat},${resultado[element].long}&key=${process.env.GOOGLE_API_KEY}`
        )
      );
    }
    console.log("sdss0", data);
